...
export async function createAppointment(req, res) {
    try {
        const { patient_name, phone, email, preferred_date, preferred_time, payment_category, aadhaar_no, razorpay_order_id, razorpay_payment_id, razorpay_signature, message, } = req.body;
        ...
        await sendClinicEmail("New appointment request (Paid)", `<h2>New Appointment</h2>
...
export async function getFullDates(req, res) {
    try {
        const maxSetting = await prisma.setting.findUnique({ where: { key: "max_appointments_per_day" } });
        const maxAppointments = maxSetting ? parseInt(maxSetting.value, 10) : 50;
